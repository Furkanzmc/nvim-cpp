#!/bin/bash
# Original script from: https://github.com/equalsraf/neovim-qt/blob/master/contrib/clang-format-diff.sh
...
if [ -n "$(git status --porcelain)" ]; then
	echo "[clang-format] ERROR: Changes reported by git, revert and retry"
	exit 1
fi
